...
func TestRetrieveToken_InParams(t *testing.T) {
	...
	defer ts.Close()
	_, _, _, err := RetrieveToken(context.Background(), clientID, "", ts.URL, url.Values{}, AuthStyleInParams)
	if err != nil {
...
func TestRetrieveTokenWithContexts(t *testing.T) {
	...
	_, _, _, err := RetrieveToken(context.Background(), clientID, "", ts.URL, url.Values{}, AuthStyleUnknown)
	if err != nil {
	...
	cancel()
	_, _, _, err = RetrieveToken(ctx, clientID, "", cancellingts.URL, url.Values{}, AuthStyleUnknown)
	close(retrieved)
	if err == nil {
